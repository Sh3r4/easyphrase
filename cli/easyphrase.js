...
var epWebConsole;
var webConsoleOutput;
var silent = false;
var effWord = (function () {
    function effWord() {
    }
    return effWord;
}());
function getWords() {
    return [
        ...
        { id: 61141, word: "subscript" },
        { id: 61142, word: "subsector" },
        { id: 61143, word: "subside" },
        { id: 61144, word: "subsiding" },
...
function getRandom(max, adjust) {
    return (Math.floor(Math.random() * max) + adjust);
}
function shuffle(unshuffledArray) {
    var shuffled = unshuffledArray;
    for (var index = unshuffledArray.length - 1; index > 0; index--) {
        var newPos = getRandom(index, 0);
...
function getWord(queryId) {
    var word = "";
    var effWords = getWords();
    for (var index = 0; index < effWords.length; index++) {
        var element = effWords[index];
...
function generatePass(quantity, lessSec) {
    if (quantity === void 0) { quantity = 1; }
    if (lessSec === void 0) { lessSec = false; }
    var _loop_1 = function () {
        var rollOutput = "";
        var words = [];
        for (var count = 0; count < 6; count++) {
            var id = generateWordId();
        ...
        if (wordsOutput.match(/,.*,/)) {
            wordsOutput = wordsOutput.replace(',', '');
        }
        if (!lessSec) {
            var antiDict = getRandom(9, 1);
            var antiDictPos = getRandom(pw.length, 0);
...
;
if (program.quantity) {
    runs = program.quantity;
}
;
if (program.lessSecure) {
    lessSecure = true;
}
generatePass(runs, lessSecure);
//# sourceMappingURL=easyphrase.js.map